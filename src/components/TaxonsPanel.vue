        ...
        <section v-if="typeof taxon !== 'undefined'" class="flex-grow-1 horizontal-flexbox">
            <div class="vertical-flexbox scroll">
                <picture-box :editable="editable"
                        ...
                        @add-photo="addTaxonPhoto"
                        @set-photo="setTaxonPhoto" 
                        @delete-photo="deleteTaxonPhoto">
                ...
                <div class="horizontal-flexbox start-align relative">
                    <collapsible-panel label="Properties" 
                            v-on:set-property="setProperty" @push-to-children="pushToChildren">
            ...
            <div class="vertical-flexbox scroll">
                ...
                <collapsible-panel label="Description">
                    <SquareTreeViewer class="large-max-width" :editable="!editable" :rootItems="itemDescriptorTree" @item-selected="onTaxonStateSelected"></SquareTreeViewer>
                </collapsible-panel>
            </div>
        </section>
    </div>
...
export default defineComponent({
    ...
    props: { taxon: Object as PropType<Taxon>, taxonsHierarchy: Hierarchy as PropType<Hierarchy<Taxon>>, descriptions: Hierarchy as PropType<Hierarchy<Character>>,
    ...
    methods: {
        ...
        onTaxonStateSelected(e: { selected: boolean, item: State }) {
            this.$emit("taxon-state-selected", e);
        },
        addTaxonPhoto(e: {detail: { value: string }}) {
            this.taxon!.photos.push(e.detail.value);
        },
        setTaxonPhoto(e: {detail: {index: number, value: string}}) {
            this.taxon!.photos[e.detail.index] = e.detail.value;
        },
        deleteTaxonPhoto(e: {detail: { index: number }}) {
