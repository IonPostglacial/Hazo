...
bunga_DetailData.escapeRegExp = function(string) {
	var _this_r = new RegExp("[.*+?^${}()|[\\]\\\\]","g".split("u").join(""));
	return string.replace(_this_r,"\\$&");
};
bunga_DetailData.findInDescription = function(description,section) {
	var re = new EReg("" + bunga_DetailData.escapeRegExp(section) + "\\s*:\\s*(.*?)(?=<br><br>)","i");
...
bunga_DetailData.removeFromDescription = function(description,sections) {
	...
	while(_g < sections.length) {
		...
		++_g;
		var re_r = new RegExp("" + bunga_DetailData.escapeRegExp(section) + "\\s*:\\s*(.*?)(?=<br><br>)","i".split("u").join(""));
...
var bunga_Character = function(id,hid,parentId,topLevel,children,data) {
...
bunga_Hierarchy.getEntries = function(hierarchy,entries,path) {
	...
	while(_g_index < _g_keys.length) {
		...
		if(entry.topLevel || path != "") {
			...
			var _g = 0;
			var _g1 = bunga_Hierarchy.forbiddenChars;
			while(_g < _g1.length) {
				var char = _g1[_g];
				++_g;
...
bunga_Taxon.fromSdd = function(taxon,extraFields,photosByRef,descriptors,statesById) {
	...
	var _g5 = [];
	var _g6 = haxe_ds_StringMap.kvIterator(descriptions_h);
	while(_g6.hasNext()) {
		var _g7 = _g6.next();
		var _ = _g7.key;
		var value = _g7.value;
	...
	return new bunga_Taxon(_g,_g1,_g2,_g3,_g4,_g6,data);
};
...
bunga_Hierarchy.forbiddenChars = [" ","*",".","\"","/","\\","[","]",":",";","|",","];
